...
estimator='EKF';
feedback='';
place = 'C:\Users\tereseva\Dropbox\Master\TereseCode\LQG\P1andW\UKF3perc\';
state = 'state';
input= 'input';
output2= 'output';
...
for k=2:nf,
    ...
    if (k==2)% || du_pc(k-1)~=0 || dwG_in(k-1)~=0 || dwL_in(k-1)~=0)
        ...
        [A,B,C,D]= v4_new_4d_linmod_knut(x0,y0,u(:,k-1),par); %Why u(:,k-1)?
...
runningTime=toc;
%sec2hour(runningTime)
run C:\Users\tereseva\Dropbox\Master\TereseCode\plottingEverything
run C:\Users\tereseva\Dropbox\Master\TereseCode\plottingReport
