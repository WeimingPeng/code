...
estimator='UKF';
feedback='';
place = 'C:\Users\tereseva\Dropbox\Master\TereseCode\PID\3percent\UKF\';
state = 'state';
input= 'input';
output2= 'output';
% **** System parameters ****
...
beta=0;
kapa=1;
landa=alpha^2*(L+kapa)-L;
gama=sqrt(L+landa);
Wc=[landa/(L+landa)+(1-alpha^2+beta) , ones(1,2*L)/(2*(L+landa))];
Wm=[landa/(L+landa) , ones(1,2*L)/(2*(L+landa))];
Wc1=ones(L,1)*Wc;
Wc2=ones(2,1)*Wc;
Wm1=ones(L,1)*Wm;
Wm2=ones(2,1)*Wm;
