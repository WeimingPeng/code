...
set(sys,'inputname',{'Z1','Z2','d1','d2'},'outputname',{'P_bh','P_wh','W_in','P_in','P_rb','DP_r','P_t','Q_out','W_out','Rho_t','Alpha_L'},'statename',{'m_Gw','m_Gp','m_Lp','m_Gr','m_Lr'});
sysc = minreal(sys);
sysm = minreal(sys({'P_in','P_t'},{'Z1','Z2'}));
...
Gd11=(Gd({'P_in','P_t'},'d1')); Gd12=(Gd({'P_in','P_t'},'d2'));
sysd = sysc({'P_in','P_t'},{'d1','d2'});
%% H-infinity ptimal controller for plant
Gc = tf(sysc({'P_bh','P_wh','W_in','P_in','P_rb','DP_r','P_t','Q_out','W_out','Rho_t','Alpha_L'},{'Z1','Z2'}));
%Gu = tf(sys('P1','Z'));
NMEAS = 2;
NCON = 2;
...
set(K,'inputname',{'P_in','P_t'},'outputname',{'Z1','Z2'});
disp('Gamma:')
disp(GAM)
...
disp('Poles of S with H-infinity optimal controller for Pbh to Z')
pole(S(1,1))
...
disp('Peak of MIMO SG11 for P_bh:')
disp(max(SG11f));
...
disp('Peak of MIMO KSI for P_bh:')
disp(max(RI1f));
...
disp('Peak of Decentralized SG11 for P_bh:')
disp(max(SG11f_dec));
...
xlim([1e-6 1e1])
title('MIMO Sensitivity and Complementary Sensitivity of P_bh')
...
xlim([1e-6 1e1])
title('Decentralized Sensitivity and Complementary Sensitivity of P_bh')
legend('|S_{11}|','|\gamma/Wp|','|T_{11}|','|\gamma/Wt|')
...
xlim([1e-6 1e1])
title('Decentralized KS(Input Usage) of P_bh')
legend('|KS_{11}|','|\gamma/Wu|')
...
%%
K = minreal(K);
sim('Hinf_MIMO_Nonlinear_Z1_PinPt_sim')
%%
t_f = t(end);
figure(7)
clf
...
plot(t/3600,rPin,'r',t/3600,Pin,'k','Linewidth',linewidth);
legend('set-point','P_{in}',1,'Orientation','Vertical')
xlim([0 t_f/3600])
ylim([-0.5 0.5])
...
plot(t/3600,rPt,'r',t/3600,Pt,'k','Linewidth',linewidth);
legend('set-point','P_{t}',1,'Orientation','Vertical')
xlim([0 t_f/3600])
