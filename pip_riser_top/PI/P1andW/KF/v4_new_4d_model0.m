...
K = sin(par.theta)/(par.A1*(1-par.Alpha_L1_av)*par.rho_L);
h1 = max(h1ss + K*(x2 - x2ss),0);
...
DP_r = rho_mix_av*par.g*par.L2 + Fric_riser;
P2_b = P2_t + DP_r;


w_G1 = par.K_g*A_g*sqrt(rho_G1*max(0,P1-Fric_pipe-P2_b));
w_L1 = (P1-Fric_pipe+par.rho_L*par.g*h1-P2_b>0)*...
    par.K_o*A_l*sqrt(par.rho_L*abs(P1-Fric_pipe+par.rho_L*par.g*h1-P2_b));
